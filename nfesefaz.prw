...
±±³   Atualizado 01/02/2021 ljjbmg  									  ³±±
±±³   Atualizado 29/03/2021 ljjbmg  									  ³±±
...
//*********************************************************************************************
//BIALE BEGIN INCLUSAO DE VARIAVEIS PARA CUSTOMIZACAO DE INFORMACOES DE IMPORTACAO 
Local cBox      := '' // Por Fagner / Biale 01-09-09  
Local aProd2    := {}
Local aPedVend  := {}
Local cCont		:= 1
Local _cXAux
//END INCLUSÃO DE VALOR DE BASE DE ICMS PARA QUANDO FOR NOTA COMPLEMENTAR DE ICMS DE VENDA 06/08/2012
...
// BIALE -- 14/10/2013 - Variavel para controle de impressão das retenções
Private lCliPublic  := .f.
...
If cTipo == "1"
	...
	// Incluido por Fagner - Biale (customização)  
		cTipoNota := "1"
	//  Fim da inclusao 
		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
			...
			If MsSeek(xFilial("SF3")+cClieFor+cLoja+cNota+cSerie)
				While !SF3->(Eof()) .And. cClieFor+cLoja+cNota+cSerie == SF3->F3_CLIEFOR+SF3->F3_LOJA+SF3->F3_NFISCAL+SF3->F3_SERIE
		            ...
		            // CUSTOMIZAÇÃO BIALE - 14/10/2013 - Gravação da variavel de controle para a impressão das retenções
			...
			ElseIf FunName() == "SPEDNFSE"
		      ...
		      // CUSTOMIZACAO - INCLUÍDO POR FAGNER / BIALE 01-09-09 (INFORMAÇÃO COMPLEMENTAR DA DANFE ALPAX)
                cBox:= SF2->F2_AXBOX 
			...
			Else
				...
				If !Empty(SF2->F2_TRANSP)
					...
					aadd(aTransp,(ALLTRIM(SA4->A4_NOME) + " ("+ ALLTRIM(SA4->A4_COD) +")"))       //Customização
					If (SA4->A4_TPTRANS <> "3")
				...
				#IFDEF TOP
					...
					//Verifica se existe Template DCL
	      			IF cVerAmb >= "4.00" .And. (ExistTemplate("PROCMSG")) //Tratativa para Grupo de Repasse de Combustiveis
						BeginSql Alias cAliasSD2
								...
								D2_ORIGLAN,D2_VALCF3,D2_VALIPI,D2_VALACRS,D2_PICM,D2_PDV,D2_BRICMSO,D2_DTVALID,D2_NUMSERI,D2_ICMRETO,D2_BRICMSD,D2_DT_VALID,D2_ICMRETD,D2_CSOSN %Exp:cField% 
								FROM %Table:SD2% SD2
								WHERE
					...
					Else
						BeginSql Alias cAliasSD2
							...
							D2_QUANT,D2_TOTAL,D2_DESCON,D2_VALFRE,D2_SEGURO,D2_PEDIDO,D2_ITEMPV,D2_DESPESA,D2_VALBRUT,D2_VALISS,D2_PRUNIT,
							D2_CLASFIS,D2_PRCVEN,D2_IDENTB6,D2_CODISS,D2_DESCZFR,D2_PREEMB,D2_DESCZFC,D2_DESCZFP,D2_LOTECTL,D2_NUMLOTE,D2_ICMSRET,D2_VALPS3,
							D2_ORIGLAN,D2_VALCF3,D2_VALIPI,D2_DTVALID,D2_NUMSERI,D2_VALACRS,D2_PICM,D2_PDV,D2_CSOSN %Exp:cField% 
							FROM %Table:SD2% SD2
							WHERE
							SD2.D2_FILIAL  = %xFilial:SD2% AND
							SD2.D2_SERIE   = %Exp:SF2->F2_SERIE% AND
				...
				//***************************************************
				// Incluído por Fagner / Biale 20/01/2011
...
//				EndIf                                  
				// Mensagem fixa solicitada pelo Ocimar em 21/06/2012
				
				cMensFix := " Declaro que os produtos perigosos estao adequadamente classificados, embalados, identificados e estivados para suportar os riscos das operacoes de transporte e que atendem as exigencias da regulamentação. "
				If ! cMensFix $ cMensCli
					cMensCli += cMensFix
				Endif
				//***************************************************									
					...
					//Customização - Alterado por Fagner -Biale em 29/05/2014 (manter estrutura anterior) 
                    /*											
						If Empty(aPedido)
					...
					*/
					...
					//****************************************************************************************
						...
						//CUSTOIZAÇÃO - ALTERADA A LINHA DA DESCRIÇÃO DO PRODUTO POR FAGNER / BIALE EM 19/01/12
			           	cDescProd :=  IIF(Empty(SC6->C6_DESCRI),ALLTRIM(SB1->B1_DESC) + " - " + ALLTRIM(SB1->B1_CAPACID),ALLTRIM(SC6->C6_DESCRI)+ " - " + ALLTRIM(SB1->B1_CAPACID))
						//****************************************************************			            
...
//BIALE BEGIN*************************************************************************************
                ...
                If !AllTrim(SC5->C5_AXENDEN) $ cMensCli
                    cMensCli += 'End. de Entrega: ' + AllTrim(SC5->C5_AXENDEN)// endereço de entrega
                EndIf
                                                
				//BEGIN ANDERSON //IDENTIFICADOR CAIXA X DANFE -----------------------------------------------------------------------
				IF !EMPTY(SF2->F2_IDENT)
					cMensBox := "  EMB.: "+ALLTRIM(SF2->F2_IDENT) + "           "
					If ! cMensBox $ cMensCli
				    cMensCli += cMensBox
					Endif 
				ENDIF                                        
				//END ANDERSON //-----------------------------------------------------------------------------------------------------
				...
				if !empty(ALLTRIM(SC5->C5_AXATEND))
				   if ascan(aAtende,ALLTRIM(SC5->C5_AXATEND)) == 0  
				   	aadd(aAtende,alltrim(SC5->C5_AXATEND)) 
				   Endif
				Endif                             
				...
				If SUBSTR(SA1->A1_CGC, 1,8) == '59179838X' .OR. SUBSTR(SA1->A1_CGC, 1,8) == '65969156X'  
				    // Customizacao realizada para o cliente Pirelli           
					//59179838004396
					//65969156000120                                              
					//BEGIN INCLUSÃO DE NÚMERO DE PEDIDO E ITEM DE PEDIDO DO PEDIDO DO CLIENTE 06/08/2012
					AADD(aPedVend,{ALLTRIM(SC6->C6_AXPEDCL),ALLTRIM(SC6->C6_AXITCLI)}) 					
					cProd2 += "<![CDATA[Pedido:"+char(124) + ALLTRIM(SC6->C6_AXPEDCL) + char(124)+ ALLTRIM(SC6->C6_AXITCLI) +char(124)+"]]>"
					//END INCLUSÃO DE NÚMERO DE PEDIDO E ITEM DE PEDIDO DO PEDIDO DO CLIENTE 06/08/2012
				Else                          
					_cXAux := '' 
					
					If 	SB1->B1_AXNONU <> 0
						_cXAux += "ONU: "  + ALLTRIM(CVALTOCHAR(SB1->B1_AXNONU))
					Endif   
...
//****************************************************************************************************************************************															                         
					...
					If !empty(SB1->B1_AXRISCO)
						_cXAux += "Classe: " + SB1->B1_AXRISCO
					Endif					
					...
					If !empty(ALLTRIM(SB1->B1_PNUMBER)) 
						_cXAux += "P.Number: " + ALLTRIM(SB1->B1_PNUMBER)
					Endif
...
// *****************************************************************************************************
					...
					If !empty((cAliasSD2)->D2_DTVALID)
						_cXAux := "Val.: " + DTOC(STOD(((cAliasSD2)->D2_DTVALID)))
					Endif
					
					If 	!empty((cAliasSD2)->D2_LOTECTL)
						_cXAux += "Lote: " + ALLTRIM((cAliasSD2)->D2_LOTECTL)
					Endif
...
// *****************************************************************************************************
					...
					If 	SB1->B1_AXSUBS = '1'
						dbSelectArea("ZA9")
						dbSetOrder(1)
						dbSeek((xFilial("ZA9") + ZA9->ZA9_COD ))
						
						While !ZA9->(EOF()) .And. ZA9->ZA9_COD == SB1->B1_COD 

							If Empty(_cXAux) 
								_cXAux += "Substância Controlada: "  + ALLTRIM(ZA9->ZA9_DESC)
							Else
								_cXAux += " \ " + ALLTRIM(ZA9->ZA9_DESC)
							EndIf
						ZA9->(dbSkip())
						End
...
// *****************************************************************************************************
					...
					If !empty((cAliasSD2)->D2_FCICOD)
						_cXAux += "Numero da FCI " + ALLTRIM((cAliasSD2)->D2_FCICOD)
					Endif
...
//*********************************************************************************************************					
					...
					If SB1->B1_AXCONC > 0
						_cXAux := "Concentracao: "  + alltrim(transform(SB1->B1_AXCONC,"@E 999.99"))
					Endif
					
					If SB1->B1_AXDENS > 0
						_cXAux += "Densidade: " + alltrim(transform(SB1->B1_AXDENS,"@E 999.99"))
					Endif
...
//*********************************************************************************************************					
					...
					If !empty(SB1->B1_MARCA)
						_cXAux := "Marca: "+ ALLTRIM(SB1->B1_MARCA)+";"
					Endif
					                
					// Alterado Por Fagner / Biale em 12/09/2014
					// Objetivo pegar o numero de série gravado na movimentação da tabela SDB, pois o sistema não está levando o numero de serie para a D2                
					// HABILITADO POR OCIMAR 05/09/2018
					
					If !empty((cAliasSD2)->D2_NUMSERI)
//						_cXAux += "N.Serie: "+ ALLTRIM((cAliasSD2)->D2_NUMSERI) // Inibido pois não está sendo gravado na SD2 - Luiz Eduardo - 04/12/2019
					Endif
				    
				    // DESABILITADO POR OCIMAR 05/09/2018
				    // RETORNADO POR LUIZ EDUARDO 04/12/2019 POIS NÃO ESTÁ GRAVANDO NA SD2
					dbselectarea("SDB") 
					SDB->(DBSETORDER(7))//DB_FILIAL+DB_PRODUTO+DB_DOC+DB_SERIE+DB_CLIFOR+DB_LOJA+DB_SERVIC+DB_TAREFA+DB_ATIVID                                                                            
					IF SDB->(DBSEEK(xFilial("SDB")+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_DOC+(cAliasSD2)->D2_SERIE+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA))
						While SDB->(!EOF()) .AND.;                                                                                                   
							xFilial("SDB")+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_DOC+(cAliasSD2)->D2_SERIE+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA == ;
							SDB->(DB_FILIAL+DB_PRODUTO+DB_DOC+DB_SERIE+DB_CLIFOR+DB_LOJA)
							 iF SUBSTR(SDB->DB_ITEM,3,2) == (cAliasSD2)->D2_ITEM .AND. SDB->DB_ITEM <> '0001'  
							 	If !empty(SDB->DB_NUMSERI)
									_cXAux += "N.Serie: "+ ALLTRIM(SDB->DB_NUMSERI)+";"                      
								Endif
							 Else                                                  
							 	If !empty(SDB->DB_NUMSERI)
									_cXAux += "N.Serie: "+ ALLTRIM(SDB->DB_NUMSERI)+";"                      
...
//*********************************************************************************************************					
					cProd2 += iif(!empty((cAliasSD2)->D2_PEDIDO)  ,"Ped. Alpax N.: " + ALLTRIM((cAliasSD2)->D2_PEDIDO)     +";","")                

					//BEGIN INCLUSÃO DE NÚMERO DE PEDIDO E ITEM DE PEDIDO DO PEDIDO DO CLIENTE 06/08/2012
					If !Empty(ALLTRIM(SC6->C6_AXPEDCL))
						cProd2 += "Ped. Cli N.:"+ ALLTRIM(SC6->C6_AXPEDCL) + "-"+ ALLTRIM(SC6->C6_AXITCLI) +";"
					EndIf                                                                                                                 
					
					AADD(aPedVend,{ALLTRIM(SC6->C6_AXPEDCL),ALLTRIM(SC6->C6_AXITCLI)})                             
					//cXped := DTOS(SC5->C5_EMISSAO) + ";"+ ALLTRIM(SC5->C5_AXPEDCL)                       
					//END INCLUSÃO DE NÚMERO DE PEDIDO E ITEM DE PEDIDO DO PEDIDO DO CLIENTE 06/08/2012
			  	Endif	
				
                // colocado por Eduardo/Biale em 02/10, msg na nota
                IF (cAliasSD2)->D2_TES = "543" 
                    aTrb := U_RADVM01(SB1->B1_COD,(cAliasSD2)->D2_QUANT,SB1->B1_UCOM)
                    IF (aTRB[1]+aTrb[2]) > 0
                        cProd2 += "BASE ICMS-ST R$ "+ALLTRIM(TRANSFORM(aTRB[1],"@E 999,999.99"));
                                 +"    VALOR ICMS-ST  R$ "+ALLTRIM(TRANSFORM(aTRB[2],"@E 999,999.99")+";")
                    ENDIF             
                ENDIF                                              
                // *******************************************************************************************
                // Customização Alpax - Policia Federal - Port. MJSP 240/19 e Siproquim 2 - Data-> 2019-05-06
                If !empty(SB1->B1_AXCTPF) 
                   cProd2 += "Produto em Conformidade a Port. MJSP 240/19 e Siproquim 2 (" ;
                          + ALLTRIM(transform ((cAliasSD2)->D2_QUANT, "@E 999,999.999")) +  " " + SB1->B1_UM + " c/ " + alltrim(SB1->B1_CAPACID) + ")"
                Endif
                // ********************************************************************************************

                // *******************************************************************************************
                // Customização Alpax - Policia Federal e SSP AND B1_AXFEDER = 'S' AND B1_AXSSP = 'S' AND B1_AXSUBS = '1'
                If SB1->B1_AXFEDER == 'S' .or. SB1->B1_AXSUBS == '1'
                   cProd2 += "Controlado PF"
                Endif
                If SB1->B1_AXSSP == 'S'
                   cProd2 += "Controlado SSP"
                Endif
                
				// ********************************************************************************************

                IF !EMPTY((cAliasSD2)->D2_NFORI)
                   cProd2 += "NF ORIGINAL "+(cAliasSD2)->D2_SERIORI+" "+(cAliasSD2)->D2_NFORI+";"
                ENDIF

   			If SUBSTR(SD2->D2_CF,2,3) $ '401/402/403/404/405/410/411/412/413'
    				DBSELECTAREA("SYD")
    				SYD->(DBSETORDER(1))
	   				If SYD->(DBSEEK(xFilial("SYD")+SB1->B1_POSIPI))
	   					iF !Empty(SYD->YD_XLEI_ST)
		                	cProd2 += ALLTRIM(SYD->YD_XLEI_ST) + ";"    				
		                Endif
    				Endif
	                SYD->(DBCLOSEAREA()) 
    			Endif
                ...
                //fim customização para Danfe : Fagner / Biale 06-08-09
				//*************************************************************************************						
						...
						ElseiF AliasIndic("CDL")
							...
							While !CDL->(Eof()) .And. CDL->CDL_FILIAL+CDL->CDL_DOC+CDL->CDL_SERIE+CDL->CDL_CLIENT+CDL->CDL_LOJA == xFilial("CDL")+(cAliasSD2)->D2_DOC+(cAliasSD2)->D2_SERIE+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA
						    	If CDL->(FieldPos("CDL_PRODNF")) <> 0 .And. CDL->(FieldPos("CDL_ITEMNF")) <> 0 .And. AllTrim(CDL->CDL_PRODNF)+AllTrim(CDL->CDL_ITEMNF) == AllTrim((cAliasSD2)->D2_COD)+AllTrim((cAliasSD2)->D2_ITEM)
								  ...
								  //Customização para nota fiscal de exportação / por Fagner - Biale 20/11/2010
									If SF2->F2_EST = "EX"
										aAdd(aDados,{"ZA02","ufEmbarq"  , IIF(!EMPTY(SF2->F2_NFEUF) , SF2->F2_NFEUF  ,"") })
							    		aAdd(aDados,{"ZA03","xLocEmbarq", IIF(!EMPTY(SF2->F2_NFELOC), SF2->F2_NFELOC ,"") })					
				    	
							    		aAdd(aExp,aDados)                                                
						    	 	 Else
				    		 			aadd(aExp,{}) //obs: No Else só existia esta instrução !
...
Else
	...
	If MsSeek(xFilial("SF1")+cNota+cSerie+cClieFor+cLoja)
		...
		//**********************************************************************************************
        // Customização - Incluído por Fagner/ Biale 04/10/2010
           if alltrim(SF1->F1_EST) = 'EX' // verifica se é cliente de Importação
           		cMensCli += "Nr. DI: " + Transform(Val(SF1->F1_NFENRDI),"@E 99/9999999-9") + " Data DI: " + DTOC(SF1->F1_NFEDTDI)
           Endif
        // Fim Inclusão
		...
		Else				
			...
			#IFDEF TOP
				...
				// Customização Alpax - Inclusão de itens na query - Fagner // ,D1_DTVALID,D1_NFEADIC,D1_NFEITAD,D1_NFEFABR,D1_ALIQII,
				BeginSql Alias cAliasSD1			
						...
						D1_VUNIT,D1_CLASFIS,D1_VALICM,D1_TIPO_NF,D1_PEDIDO,D1_ITEMPC,D1_VALIMP5,D1_VALIMP6,D1_BASEIRR,D1_VALIRR,D1_LOTECTL, D1_DTVALID,D1_NFEADIC,D1_NFEITAD,D1_NFEFABR,D1_ALIQII,
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
				...
				//*******************************************************************
				//BIALE BEGIN CUSTOMIZACAO PARA INCLUSAO DE INFORMACOES DE IMPORTACAO
				If Len(SF1->F1_NFENRDI) > 0 .AND. ALLTRIM(SF1->F1_NFENRDI) <> ""
					cAlpax += '<DI>'
					cAlpax += '<nDI>'+ALLTRIM(SF1->F1_NFENRDI)+'</nDI>'
					cAlpax += '<dtDi>'+SUBSTR(DTOS(SF1->F1_NFEDTDI),1,4)+'-'+SUBSTR(DTOS(SF1->F1_NFEDTDI),5,2)+'-'+SUBSTR(DTOS(SF1->F1_NFEDTDI),7,2)+ '</dtDi>'      
					cAlpax += '<LocDesemb>'+ALLTRIM(SF1->F1_NFELOC)+'</LocDesemb>'
					cAlpax += '<UFDesemb>'+ALLTRIM(SF1->F1_NFEUF)+ '</UFDesemb>'
					cAlpax += '<dtDesemb>'+SUBSTR(DTOS(SF1->F1_NFEDES),1,4)+'-'+SUBSTR(DTOS(SF1->F1_NFEDES),5,2)+'-'+SUBSTR(DTOS(SF1->F1_NFEDES),7,2)+ '</dtDesemb>'					
					cAlpax += '<Exportador>'+ALLTRIM(SF1->F1_FORNECE)+ '</Exportador>'      

					If cVerAmb >= "3.10"
						cAlpax += '<viaTransp>'+ConvType(SF1->F1_VTRANS,2)+ '</viaTransp>'
						cAlpax += NfeTag('<AFRMM>',ConvType(SF1->F1_VAFRMM,15,2))
						cAlpax += '<intermedio>'+ConvType(SF1->F1_INTERM,1)+ '</intermedio>'
						cAlpax += NfeTag('<CNPJ>',ConvType(SF1->F1_CNPJAE,14))
						cAlpax += NfeTag('<UfTerceiro>',ConvType(SF1->F1_UFTERC,2))		        
					EndIf
					
					If Len((cAliasSD1)->D1_NFEADIC) > 0 
						cAlpax += '<adicao>'
						cAlpax += '<Adicao>'+CVALTOCHAR(VAL(ALLTRIM((cAliasSD1)->D1_NFEADIC)))+ '</Adicao>'
						cAlpax += '<SeqAdic>'+CVALTOCHAR(VAL(ALLTRIM((cAliasSD1)->D1_NFEITAD)))+ '</SeqAdic>'
						cAlpax += '<Fabricante>'+ALLTRIM((cAliasSD1)->D1_NFEFABR)+ '</Fabricante>'
						cAlpax += '</adicao>'
					EndIf 
					cAlpax += '</DI>'		
					nAlpaxII   :=  (cAliasSD1)->D1_II     
					nAlpaxTII  +=  (cAliasSD1)->D1_II     //ACUMULA VALORES DE II                  
					nAlpaxBII  :=  ((cAliasSD1)->D1_II / (cAliasSD1)->D1_ALIQII) * 100  
					nAlpaxBTII +=  nAlpaxBII  
					AADD(aAlpax,{cAlpax, nAlpaxII, nAlpaxBII})
					cAlpax := ""

				EndIf                                         
				aadd(apedvend,{"",""})
				//BIALE END CUSTOMIZACAO PARA INCLUSAO DE INFORMACOES DE IMPORTACAO
				//*******************************************************************
...
//Inicio customização para Danfe : Fagner / Biale - 06-08-09
                ...
                cProd2 := ''
                cProd2 += iif(!empty(ALLTRIM(SB1->B1_PNUMBER)),"P.Number: " + ALLTRIM(SB1->B1_PNUMBER)           +";","")
                cProd2 += iif(!empty((cAliasSD1)->D1_LOTECTL) ,"Lote: "     + ALLTRIM((cAliasSD1)->D1_LOTECTL)   +";","")
                cProd2 += iif(!empty(SD1->D1_DTVALID) ,"Val.: "     + DTOC(STOD(SD1->D1_DTVALID))," ")
                cProd2 += iif(!empty(SB1->B1_AXNONU)          ,"  N.ONU: "  +ALLTRIM(CVALTOCHAR(SB1->B1_AXNONU))+";"," ")
                
                cProd2 += iif(!empty(SD1->D1_DTVALID) .and.  empty(SB1->B1_AXNONU),";"," ")
                
                cProd2 += iif(cGrupoEmb <> ''                 ,"Emb.: "       +cGrupoEmb                             ," ")
                cProd2 += iif(!empty(SB1->B1_AXRISCO)         ,"  Cl.Risco: " +SB1->B1_AXRISCO                    +";","")
				//cProd2 += iif(!empty(SC5->C5_AXPEDCL)         ,"Pedido N.: " + ALLTRIM(SC5->C5_AXPEDCL)           +";","")
                                  
			    // buscando nota original
                IF !EMPTY((cAliasSD1)->D1_NFORI)
                      cProd2 += "NF Original: "+(cAliasSD1)->D1_NFORI+" "+(cAliasSD1)->D1_SERIORI+";"                               
                ENDIF                               
                ...
                If !empty(SB1->B1_AXCTPF) 
                   cProd2 += "Produto em Conformidade a Port. MJSP 240/19 e Siproquim 2 (" ;
                          + ALLTRIM(transform ((cAliasSD1)->D1_QUANT, "@E 999,999.999")) +  " " + SB1->B1_UM + " c/ " + alltrim(SB1->B1_CAPACID) + ")"
                Endif
				// *******************************************************************************************
				            
                aadd(aProd2,{ALLTRIM(cProd2)})					
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////					        
			...
			//**********************************************************************************
			// Customizacao Fagner - Biale 11/04/13 - Mensagem de Informacao sobre devolucao de desconto
			If SF1->F1_DESCONT > 0
				cMensDesc := "Desconto R$ " + ALLTRIM(Transform(SF1->F1_DESCONT,"@E 999,999,999.99"))
				
				If ! cMensDesc $ cMensFis
				     cMensFis += cMensDesc
				Endif
			Endif                         
			// Fim da customizacao 
...
If !Empty(aNota)
	...
	For nX := 1 To Len(aProd)
		If nLenaIpi > 0
			...
			EndIf
		EndIf
		//CUSTOMIZACAO - ALPAX -INCLUÍDO PARÂMETRO aProd2 no array : Fagner / Biale 11-08-09
		//BEGIN INCLUSÃO DE NÚMERO DE PEDIDO E ITEM DE PEDIDO DO PEDIDO DO CLIENTE 06/08/2012
    	//BEGIN INCLUIDO DEVIDO NFE PODER UTILIZAR A MESMA ROTINA 15/06/2012		
		cString += 	NfeItem(aProd[nX],aICMS[nX],aICMSST[nX],aIPI[nX],aPIS[nX],aPISST[nX],aCOFINS[nX],aCOFINSST[nX],aISSQN[nX],aCST[nX],;
					...
					@aTotICMSST,len(aProd), nX, @nValDifer,aProd2[nX,1],aPedvend[nX])
	Next nX
	...
	//INCLUÍDO PARÂMETRO cBox na chamada da função abaixo por Fagner / Biale 01-09-09 - CUSTOMIZACAO 					
...
If aProd[8] <> aProd[11]  // aProd[8] = B1_UM  e  aProd[11] = B5_UMDIPI - pega diferente para segunda unidade de medida                                                                
	cString += '<uTrib>'+ConvType(aProd[08])+'</uTrib>'
	cString += '<qTrib>' + ConvType(aProd[09], 15, 4) + '</qTrib>'                                          
	cString += '<vUnTrib>'+ IIf(cF2Tipo == "C",ComplPreco(cTipo,cF2Tipo,aProd),ConvType(aProd[10]/aProd[09],21,8))+'</vUnTrib>'	
Else
cString += '<uTrib>'+ConvType(aProd[11])+'</uTrib>'
...
//***********************************************************************
//BIALE BEGIN INCLUSAO DE INFORMACOES DE IMPORTACAO PARA GERACAO DA DANFE - customizaÇÃO alpax
If cAlpaxtip <> "1"        
	nAlpax++
	If len(aAlpax) > 0 .AND. nAlpax <= len(aAlpax) 
		cString += aAlpax[nAlpax,1]
	Endif
Endif
//BIALE END INCLUSAO DE INFORMACOES DE IMPORTACAO PARA GERACAO DA DANFE - customizaÇÃO alpax
//***********************************************************************
...
//Customização Incluído por Fagner - Biale em 31/05/2011
//BEGIN INCLUSÃO DE NÚMERO DE PEDIDO E ITEM DE PEDIDO DO PEDIDO DO CLIENTE 06/08/2012

IF Len(aPedVend) > 0        

	If !empty(aPedVend[1]) .AND. Len(aPedCom) == 0
		cString += '<xPed>'+ConvType(aPedVend[1])+'</xPed>'
		cString += '<nItemPed>'+ConvType(aPedVend[2])+'</nItemPed>'
	Endif
ENDIF
//END INCLUSÃO DEVIDO ROTINA PODE SER UTILIZADA PELA NOTA DE SAIDA 15/06/2012
...
//***********************************************************************
// BIALE BEGIN Tratamento de imposto de importacao CUSTOMIZADA PARA A ALPAX      
If cAlpaxtip <> "1" .AND. nAlpax <= len(aAlpax) .AND. nALPAX <> 0
//	If nAlpaxII > 0 .OR. nAlpaxBII > 0	        
      
	If aAlpax[nAlpax,2] > 0  .or. aAlpax[nAlpax,3] > 0 
		cString += '<imposto>'
		cString += '<codigo>II</codigo>'
		cString += '<Tributo>'
		cString += '<vBC>'      +ConvType(aAlpax[nAlpax,3],15,2)+'</vBC>'
		cString += '<Valor>'    +ConvType(aAlpax[nAlpax,2],15,2)+'</Valor>'
		cString += '</Tributo>'			
		cString += '<cpl>'
		cString += '<vDespAdu></vDespAdu>'
		cString += '<vIOF></vIOF>'
		cString += '</cpl>'						
		cString += '</imposto>'
	EndIf
Endif 
// BIALE END Tratamento de imposto de importacao CUSTOMIZADA PARA A ALPAX      
//***********************************************************************
...
Return(cString)
// Customizaçao alpax - Incluido o parâmetro cBox  por Fagner / Biale 01-09-09
...
// Customização - ALTERADO POR FAGNER - Biale 29-07-10  

cString += '<Cpl>'                
      if cTipoNota = '1'
            if len(alltrim(SF2->F2_AXBOX)) > 0 
                cString +='Box: ' + alltrim(SF2->F2_AXBOX) + '  '
            Endif                                                
            
            if len(aAtende) >= 1
            	cString +=' - Atendente(s): '
	            For aNV := 1 to len(aAtende)      
					cString += upper(aAtende[aNV]) + ' / '
	            Next        
	            cString:= SubStr(cString, 1, len(cString) -2) + ' - '
	     	Endif
      Endif                                 
      
cString += '[ContrTSS='+StrZero(Year(ddatabase),4)+'-'+StrZero(Month(ddatabase),2)+'-'+StrZero(Day(ddatabase),2)+'#'+AllTrim(Time())+'#'+AllTrim(SubStr(cUsuario,7,15))+']'
...
// Customizado por Fagner - Biale  em 14/10/2013 
// Objetivo: Mostrar as retenções dos impostos apenas para os clientes onde A1_TPESSOA = EP (Empresa Publica) 
If Len(aRet) > 0 .And. lImpRet .AND. lCliPublic
...
// CUSTOMIZAÇÃO 
cString +="FISPQ´S DISPONÍVEIS NO SITE: www.alpax.com.br/fispq.htm"       
// FIM CUSTOMIZAÇÃO	
