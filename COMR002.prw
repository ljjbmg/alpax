...
±±ºPrograma  ³COMR002   ºAutor  ³Microsiga           º Data ³  22/09/07   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³ RELATORIO DE ITENS EM PONTO DE PEDIDO CONSIDERANDO PEDIDOS º±±
±±º          ³ DE VENDAS LIBERADOS E LIBERADOS COM BLOQUEIO.ALMOX.02      º±±
...
Private cPict          := ""
Private titulo       := "Ponto de Pedido ALMOX.02"
Private nLin         := 80
...
_cQuery += "       B2_SALPEDI AS 'PED_PEND',(B2_QPEDVEN - B2_QATU - B2_SALPEDI) AS COMPRA "
_cQuery += "       FROM " + RetSqlName("SB2") + " B2 "
_cQuery += "                INNER JOIN " + RetSqlName("SB1") + " B1 "
_cQuery += "                    ON B1_FILIAL = '" + xFilial("SB1") + "'  AND B1_COD = B2_COD "
_cQuery += "       WHERE B2_FILIAL = '" + xFilial("SB2") + "' AND B2_LOCAL = '02'  "
_cQuery += "             AND B1.D_E_L_E_T_ = ' ' AND B2.D_E_L_E_T_ = ' ' "
_cQuery += "GROUP BY B1_PNUMBER, B1_DESC, B1_CAPACID, B1_MARCA, B2_QATU, B2_SALPEDI, B1_COD, B2_QPEDVEN, B2_QTNP, B2_QNPT, B2_RESERVA "
_cQuery += "      HAVING (B2_QPEDVEN - B2_QATU - B2_SALPEDI) > 0 "
