		...
		If DbSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)	
           ...
           If cMvMsgTrib $ "1-3"
			...
			While !(cAliasSD2)->(Eof()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
				...
				cNatOper := ALLTRIM(SB1->B1_DESC)+chr(13)+chr(10)
				...
				If dbSeek(xFilial("SE1")+SF2->F2_CLIENTE+SF2->F2_LOJA+SF2->F2_SERIE+SF2->F2_DOC)
					...
					xFilial("SE1")+SE1->E1_CLIENTE+SE1->E1_LOJA+SE1->E1_PREFIXO+SE1->E1_NUM
						IF SE1->E1_TIPO = "NF"
							//cNatOper += CHR(10) + CHR(13) + "Titulo: " + SE1->E1_NUM +"/"+SE1->E1_PARCELA + " Vencto.:" + DTOC(SE1->E1_VENCREA )
							cNatOper += CHR(10) + CHR(13) + "Titulo: " + SE1->E1_NUM +"/"+SE1->E1_PARCELA + " Vencto.:" + DTOC(SE1->E1_VENCORI )
						ENDIF
						SE1->(dbSkip())
					EndDo
				EndIf

				If !empty(SC5->C5_MENSERV)  .and. !(alltrim(SC5->C5_MENSERV) $ cNatOper)
					cNatOper += CHR(10) + CHR(13) + ALLTRIM(SC5->C5_MENSERV)
				Endif

				//Incluso da mensagem do pedido como informaes no corpo da nota- Por Fagner em 05/01/2012

				If !empty(SC5->C5_AXPEDCL).and. !(alltrim(SC5->C5_AXPEDCL) $ cNatOper)
					cNatOper += CHR(10) + CHR(13) + "Num. Ped. Cliente: "+ALLTRIM(SC5->C5_AXPEDCL)
				Endif     
				...
				// FIM ALTERADO PARA TRATAR MENSAGENS ALPAX
