...
#DEFINE HMARGEM   030
#DEFINE VMARGEM   030
#DEFINE MAXITEM   013        //Andre R. Esteves 04/12/2018           // Máximo de produtos para a primeira página
...
#DEFINE MAXITEMC  050      //Andre R. ESteves 10/12/2018             // Máxima de caracteres por linha de produtos/serviços
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
								...
								//CUSTOMIZAÇAO GERAÇÃO DE PDF AUTOMÁTICO - POR: FAGNER / BIALE  19/08/2013

								//VERIFICA SE A NOTA É DE ENTRADA OU SAÍDA e PEGA O E-MAIL E MONTA NOME DO ARQUIVO PARA ENVIO DA DANFE
 									If aNotas[nX][02]=="E"                                     
										dbSelectArea("SA2")
										SA2->(dbSetOrder(1))
										SA2->(dbSeek(xFilial("SA2")+SF1->F1_FORNECE+SF1->F1_LOJA))
									    cClifor  := SA2->A2_EMAIL                 
									    cNomArq  := alltrim(SF2->F2_DOC + "-" + SF1->F1_SERIE)
										cFileName:= AllTrim(SF2->F2_DOC) + "-" + AllTrim(SF1->F1_SERIE)										
									Else
									 	dbSelectArea("SA1")            
									 	SA1->(dbSetOrder(1))
										SA1->(dbSeek(xFilial("SA1")+SF2->F2_CLIENTE+SF2->F2_LOJA))
									    cClifor  := SA1->A1_EMAIL
									    cNomArq  := alltrim(SF2->F2_SERIE+SF2->F2_DOC)
										cFileName:= AllTrim(SF2->F2_SERIE) + AllTrim(SF2->F2_DOC)
									Endif                                                                 
									

									oPrintPDF := Nil            
									lPreview := .T.
									oPrintPDF := FWMSPrinter():New(cFileName+".PDF",6,.F.,,.T.,,,,,,,.F.)
									oPrintPDF:SetResolution(78) //Tamanho estipulado para a Danfe
								   //	oPrintPDF:SetPortrait() 
								    oPrintPDF:SetLandScape() 
									oPrintPDF:SetPaperSize(DMPAPER_A4)
									oPrintPDF:SetMargin(60,60,60,60)
									oPrintPDF:cPathPDF := "C:\DANFE\" 

									ImpDet(@oPrintPDF,oNFe,cAutoriza,cModalidade,oNfeDPEC,cCodAutDPEC,aXml[nX][6],aXml[nX][7],aNotas[nX])        
									...
									//RENOMEIA O ARQUIVO 
									FRename( "C:\DANFE\"+ cFileName + ".PDF", "C:\DANFE\"+cNomArq + ".PDF")
									cNewfile := "C:\DANFE\"+cNomArq + ".PDF" 
									
								 //	While !file(cNewfile)
							     //      Loop
							     //   Enddo
									
									cAnexDanfe := "\NOTAS\"+cNomArq + ".PDF"                                                                                    

					
									CPYT2S(cNewFile,"\NOTAS\",.F.)
										
								 //	While !file(cAnexDanfe)
							     //      loop
								 //   Enddo
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
								...
								//CUSTOMIZAÇAO para envio da DANFE em paisagem - POR: JP / TOTALIT 19/03/19

								U_ENVMAIL(,cClifor,,,cAnexDanfe)
...
nValII	 := Transform(Val(oTotal:_ICMSTOT:_vII:TEXT),		"@e 999,999,999,999.99") // Customizado Marcio
nICMSRem := Transform(Val(oTotal:_ICMSTOT:_vICMSUFRemet:TEXT),		"@e 999,999,999,999.99") // Customizado Marcio
nICMSDst := Transform(Val(oTotal:_ICMSTOT:_vICMSUFDest:TEXT),		"@e 999,999,999,999.99") // Customizado Marcio
...
/* ----------------CUSTOMIZACAO MARCIO----------------*/
oDanfe:Box(nLine+328,nBaseCol+30,nLine+353,180)//oDanfe:Box(nLine+328,nBaseCol+30,nLine+353,262)
oDanfe:Say(nLine+336,nBaseTxt,"BASE CALC. ICMS",oFont08N:oFont)
If cMVCODREG $ "2|3" 
...
oDanfe:Box(nLine+328,180,nLine+353,260)//oDanfe:Box(nLine+328,262,nLine+353,402)
oDanfe:Say(nLine+336,190,"VALOR DO ICMS",oFont08N:oFont)//oDanfe:Say(nLine+336,272,"VALOR DO ICMS",oFont08N:oFont)
If cMVCODREG $ "2|3" 
	oDanfe:Say(nLine+346,190,aTotais[02],oFont08:oFont)//oDanfe:Say(nLine+346,272,aTotais[02],oFont08:oFont)
ElseIf lImpSimpN
	oDanfe:Say(nLine+346,190,aSimpNac[02],oFont08:oFont)//oDanfe:Say(nLine+346,272,aSimpNac[02],oFont08:oFont)
Endif

oDanfe:Box(nLine+328,260,nLine+353,340)//oDanfe:Box(nLine+328,402,nLine+353,557)
oDanfe:Say(nLine+336,270,"BASE CALC. ICMS ST",oFont08N:oFont)//oDanfe:Say(nLine+336,412,"BASE DE CALCULO DO ICMS ST",oFont08N:oFont)
oDanfe:Say(nLine+346,270,aTotais[03],oFont08:oFont)//oDanfe:Say(nLine+346,412,aTotais[03],oFont08:oFont)

oDanfe:Box(nLine+328,340,nLine+353,420)//oDanfe:Box(nLine+328,557,nLine+353,697)
oDanfe:Say(nLine+336,350,"VALOR ICMS SUBST.",oFont08N:oFont)//oDanfe:Say(nLine+336,567,"VALOR DO ICMS SUBSTITUIÇÃO",oFont08N:oFont)
oDanfe:Say(nLine+346,350,aTotais[04],oFont08:oFont)//oDanfe:Say(nLine+346,567,aTotais[04],oFont08:oFont)

oDanfe:Box(nLine+328,420,nLine+353,500)
oDanfe:Say(nLine+336,430,"V.IMP.IMPORTAÇÃO",oFont08N:oFont)
oDanfe:Say(nLine+346,430,nValII,oFont08:oFont)

oDanfe:Box(nLine+328,500,nLine+353,580)
oDanfe:Say(nLine+336,510,"V.ICMS UF REMET.",oFont08N:oFont)
oDanfe:Say(nLine+346,510,nICMSRem,oFont08:oFont)

nValFECP := 0
DbSelectArea("CD2")
CD2->(DbSetOrder(1))//CD2_FILIAL, CD2_TPMOV, CD2_SERIE, CD2_DOC, CD2_CODCLI, CD2_LOJCLI, CD2_ITEM, CD2_CODPRO, CD2_IMP, R_E_C_N_O_, D_E_L_E_T_
 If CD2->(DbSeek(SF2->F2_FILIAL + "S" + SF2->F2_SERIE + SF2->F2_DOC + SF2->F2_CLIENTE + SF2->F2_LOJA))
	While CD2->(!EOF()) .And. CD2->CD2_FILIAL == SF2->F2_FILIAL .And.; 
		CD2->CD2_TPMOV == "S" .And.; 
		CD2->CD2_SERIE == SF2->F2_SERIE .And.; 
		CD2->CD2_DOC == SF2->F2_DOC .And.;
		CD2->CD2_CODCLI == SF2->F2_CLIENTE .And.; 
		CD2->CD2_LOJCLI == SF2->F2_LOJA
		 	
		If CD2->(FieldPos("CD2_VFCP")) > 0 .and. CD2->CD2_VFCP > 0
			nValFECP += CD2->CD2_VFCP 
		Endif  
		CD2->(DbSkip())
	EndDo
Endif

oDanfe:Box(nLine+328,580,nLine+353,660)
oDanfe:Say(nLine+336,590,"VALOR DO FCP",oFont08N:oFont)
//oDanfe:Say(nLine+346,590,Transform(SF3->F3_VALFECP,"@e 999,999,999,999.99"),oFont08:oFont)//F3_VFECPST
oDanfe:Say(nLine+346,590,Transform(nValFECP,"@e 999,999,999,999.99"),oFont08:oFont)//F3_VFECPST

oDanfe:Box(nLine+328,660,nLine+353,740)
oDanfe:Say(nLine+336,670,"VALOR DO PIS",oFont08N:oFont)
//oDanfe:Say(nLine+346,670,Transform(SF2->F2_VALPIS,"@e 999,999,999,999.99"),oFont08:oFont)//F3_VALIMP6
oDanfe:Say(nLine+346,670,Transform(SF2->F2_VALIMP6,"@e 999,999,999,999.99"),oFont08:oFont)

oDanfe:Box(nLine+328,740,nLine+353,MAXBOXH+70)//oDanfe:Box(nLine+328,697,nLine+353,MAXBOXH+70)
oDanfe:Say(nLine+336,750,"V.TOTAL PRODUTOS",oFont08N:oFont)//oDanfe:Say(nLine+336,707,"VALOR TOTAL DOS PRODUTOS",oFont08N:oFont)
oDanfe:Say(nLine+346,750,aTotais[05],oFont08:oFont)//oDanfe:Say(nLine+346,707,aTotais[05],oFont08:oFont)
 

oDanfe:Box(nLine+353,nBaseCol+30,nLine+378,180)//oDanfe:Box(nLine+353,nBaseCol+30,nLine+378,232)
oDanfe:Say(nLine+361,nBaseTxt,"VALOR DO FRETE",oFont08N:oFont)
oDanfe:Say(nLine+371,nBaseTxt,aTotais[06],oFont08:oFont) 

oDanfe:Box(nLine+353,180,nLine+378,260)//oDanfe:Box(nLine+353,232,nLine+378,352)
oDanfe:Say(nLine+361,190,"VALOR DO SEGURO",oFont08N:oFont)//oDanfe:Say(nLine+361,242,"VALOR DO SEGURO",oFont08N:oFont)
oDanfe:Say(nLine+371,190,aTotais[07],oFont08:oFont)//oDanfe:Say(nLine+371,242,aTotais[07],oFont08:oFont)  

oDanfe:Box(nLine+353,260,nLine+378,340)//oDanfe:Box(nLine+353,352,nLine+378,452)
oDanfe:Say(nLine+361,270,"DESCONTO",oFont08N:oFont)//oDanfe:Say(nLine+361,362,"DESCONTO",oFont08N:oFont)
oDanfe:Say(nLine+371,270,aTotais[08],oFont08:oFont) //oDanfe:Say(nLine+371,362,aTotais[08],oFont08:oFont) 

oDanfe:Box(nLine+353,340,nLine+378,420)  //oDanfe:Box(nLine+353,452,nLine+378,592)
oDanfe:Say(nLine+361,350,"OUTRAS DESPESAS",oFont08N:oFont) //oDanfe:Say(nLine+361,462,"OUTRAS DESPESAS ACESSÓRIAS",oFont08N:oFont)
oDanfe:Say(nLine+371,350,aTotais[09],oFont08:oFont) //oDanfe:Say(nLine+371,462,aTotais[09],oFont08:oFont)    

oDanfe:Box(nLine+353,420,nLine+378,500)   //oDanfe:Box(nLine+353,592,nLine+378,712)
oDanfe:Say(nLine+361,430,"VALOR TOTAL IPI",oFont08N:oFont) //oDanfe:Say(nLine+361,602,"VALOR TOTAL DO IPI",oFont08N:oFont)
oDanfe:Say(nLine+371,430,aTotais[10],oFont08:oFont)  //oDanfe:Say(nLine+371,602,aTotais[10],oFont08:oFont)    

oDanfe:Box(nLine+353,500,nLine+378,580)   
oDanfe:Say(nLine+361,510,"V.ICMS UF DEST.",oFont08N:oFont) 
oDanfe:Say(nLine+371,510,nICMSDst,oFont08:oFont) 

oDanfe:Box(nLine+353,580,nLine+378,660)   
oDanfe:Say(nLine+361,590,"V.TOT.TRIB",oFont08N:oFont) 
oDanfe:Say(nLine+371,590,Transform(SF2->F2_TOTIMP,"@e 999,999,999,999.99"),oFont08:oFont)  

oDanfe:Box(nLine+353,660,nLine+378,740)   
oDanfe:Say(nLine+361,670,"VALOR DA COFINS",oFont08N:oFont) 
//oDanfe:Say(nLine+371,670,Transform(SF2->F2_VALCOFI,"@e 999,999,999,999.99"),oFont08:oFont) //F3_VALIMP5
oDanfe:Say(nLine+371,670,Transform(SF2->F2_VALIMP5,"@e 999,999,999,999.99"),oFont08:oFont) 

oDanfe:Box(nLine+353,740,nLine+378,MAXBOXH+70) //oDanfe:Box(nLine+353,712,nLine+378,MAXBOXH+70)
oDanfe:Say(nLine+361,750,"V.TOTAL DA NOTA",oFont08N:oFont)  //oDanfe:Say(nLine+361,722,"VALOR TOTAL DA NOTA",oFont08N:oFont)
oDanfe:Say(nLine+371,750,aTotais[11],oFont08:oFont)  //oDanfe:Say(nLine+371,722,aTotais[11],oFont08:oFont) 
/* -------------------------------------------------*/   
...
//oDanfe:Say(nLine+397,380,SA4->A4_COD,oFont08:oFont)       //OCIMAR 07/02/19
...
//Andre R. ESteves - redefinindo colunas
aTamCol[2] := 214  //Descr Prod
aTamCol[10]:= 0
aTamCol[11]:= 0
aTamCol[12]:= 0
...
nBaseTxt := 180
// ************************************************************
// Customização por Fagner / Biale em 19/08/2013
// Inclusão dos dados do endereço eletrônico e e-mail da Alpax
// ************************************************************
...
Endif
*/ 
// Inicio da Customização - Fagner / Biale
oDanfe:Box(nLine+042,nBaseCol,nLine+139,450)
oDanfe:Say(nLine+057,nBaseTxt, "Identificação do emitente",oFont12N:oFont)
If len(oEmitente:_xNome:Text)>43
	oDanfe:Say(nLine+070,nBaseTxt,SubStr(NoChar(oEmitente:_xNome:Text,lConverte),1,45), oFont12N:oFont )
...
Else
	If Type("oEmitente:_EnderEmit:_xCpl") <> "U"
		...
		oDanfe:Say(nLine+110,nBaseTxt, oEmitente:_EnderEmit:_xMun:Text+"/"+oEmitente:_EnderEmit:_UF:Text,oFont08N:oFont)
		oDanfe:Say(nLine+120,nBaseTxt, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont)
		oDanfe:Say(nLine+130,098, "Site: www.alpax.com.br / E-Mail: alpax@alpax.com.br",oFont08N:oFont)
//		oDanfe:Say(nLinCalc,098, "E-Mail: alpax@alpax.com.br",oFont08N:oFont)*/
	Else
		oDanfe:Say(nLine+090,nBaseTxt, NoChar(oEmitente:_EnderEmit:_xBairro:Text,lConverte)+" Cep:"+TransForm(IIF(Type("oEmitente:_EnderEmit:_Cep")=="U","",oEmitente:_EnderEmit:_Cep:Text),"@r 99999-999"),oFont08N:oFont)
		oDanfe:Say(nLine+100,nBaseTxt, oEmitente:_EnderEmit:_xMun:Text+"/"+oEmitente:_EnderEmit:_UF:Text,oFont08N:oFont)
		oDanfe:Say(nLine+110,nBaseTxt, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont)
...
// Fim  da Customização - Fagner / Biale
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
//³Quadro 2                                                                ³
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
...
If nFolha == 1   
	...
	If nMAXITEM <> 0
		If ( valType(oEntrega)=="U" ) .and. ( valType(oRetirada)=="U")
			//oDanfe:FillRect({nLine+455,nBaseCol,nLine+574,nBaseCol+30},oBrush)  //ANDRE R. ESTEVES - 04/12/2018
			oDanfe:FillRect({nLine+455,nBaseCol,nLine+598,nBaseCol+30},oBrush)
		...
		//oDanfe:Box(nLine+454,nBaseCol+31,nLine+574,MAXBOXH+70) // ANDRE R. ESTEVES - 04/12/2018
		oDanfe:Box(nLine+454+nAjustaPro,nBaseCol+31,nLine+599,MAXBOXH+70)
		...
		//	nAux += aTamCol[10]
//	oDanfe:Box(nLine+454, nAux, nLine+469, nAux + aTamCol[10])  //Andre R. Esteves
...
//	oDanfe:Say(nLine+462, nAux + 2,"TOTAL LIQ", oFont08N:oFont)
		...
		For Nx :=1 to Len(aColProd)
			If ( valType(oEntrega)=="U" ) .and. ( valType(oRetirada)=="U")
				...
				oDanfe:Box(nLine+469,aColProd[nX][1],nLine+599,aColProd[nX][2])  //ANDRE R. ESTEVES - 04/12/2018
			ElseIf ( valType(oEntrega)=="O" ) .or. ( valType(oRetirada)=="O")
				//oDanfe:Box(nLine+469+nAjustaPro,aColProd[nX][1],nLine+575+20,aColProd[nX][2])		
				oDanfe:Box(nLine+469+nAjustaPro,aColProd[nX][1],nLine+599+20,aColProd[nX][2])	//ANDRE R. ESTEVES - 02/12/2019	
			Endif
		Next
	EndIf
Else
...
/************ Biale Eduardo 23/05/2014
BEGIN BIALE - customização alpax 
Impressao de embalagem para uso na expedicao
*/             
aAreaF2 := SF2->(GETAREA())
DBSELECTAREA("SF2")
DBSETORDER(1)
IF DBSEEK(XFILIAL("SF2")+alltrim(StrZero(Val(oIdent:_NNf:Text),9))+alltrim(oIdent:_Serie:Text))
    nLin:= nLin+3
    oDanfe:Say(nLin,102,"EMB: "+ALLTRIM(SF2->F2_IDENT),OFONT12N:oFont)
        nLin:= nLin+15
ENDIF  
RestArea(aAreaF2)
