...
//*********************************************************************************************    
	For ncw := 1 To mv_par09	
		...
		If mv_par14 == 1 // 1 = Sim (Enviar ao fornecedor?)
			cMailFor := Alltrim(SA2->A2_EMAIL)
			cNomeFor := Alltrim(SA2->A2_NOME)
			
			//If lAprov
				WFtoFor(NumPed,cMailFor,cNomeFor,cDir,cArq)	
			//Else
			//	MsgAlert('Esse pedido n�o est� aprovado e n�o ser� enviado ao fornecedor.','PC n�o aprovado')
			//Endif
		Endif	
	Next
...
If MV_PAR07 == 2 .And. !Empty(SC7->C7_SEGUM)
   oPrn:Say( li, /*1500*/ CalcCol(1550) , SC7->C7_SEGUM ,oFont6,100,,,2 )
Else
   oPrn:Say( li, /*1510*/ CalcCol(1550) , SC7->C7_UM ,oFont6,100,,,2 )
EndIf

If MV_PAR07 == 2 .And. !Empty(SC7->C7_QTSEGUM) 
   oPrn:Say( li,  CalcCol(1620) , Transform(SC7->C7_QTSEGUM,/*PesqPict("SC7","C7_QUANT",14,mv_par12)*/PesqPict("SC7","C7_QUANT")) ,oFont6c,100,,,2 ) // 1400
Else
   oPrn:Say( li, CalcCol(1620) , Transform(SC7->C7_QUANT,/*PesqPict("SC7","C7_QUANT",14,mv_par12)*/ PesqPict("SC7","C7_QUANT")) ,oFont6c,100,,,2 )  // 1400
EndIf

If MV_PAR07 == 2 .And. !Empty(SC7->C7_QTSEGUM)  
   oPrn:Say( li,  CalcCol(1880)  , Transform(xMoeda((SC7->C7_TOTAL/SC7->C7_QTSEGUM),SC7->C7_MOEDA,MV_PAR12,SC7->C7_DATPRF),/*PesqPict("SC7","C7_FRETE",14, mv_par12)*/PesqPict("SC7","C7_FRETE")) ,oFont6c,100/*,,,2*/ ) 
Else
   oPrn:Say( li, CalcCol(1880)   , Transform(xMoeda(SC7->C7_PRECO,SC7->C7_MOEDA,MV_PAR12,SC7->C7_DATPRF),/*PesqPict("SC7","C7_FRETE",14,mv_par12)*/PesqPict("SC7","C7_FRETE")) ,oFont6c,100/*,,,2*/ )
EndIf
                             
//oPrn:Say( li, 1630, Transform(SC7->C7_DESC1,PesqPict("SC7","C7_DESC1",16,mv_par12)) ,oFont6c,100 )
oPrn:Say( li, CalcCol(2180) 	, Transform(SC7->C7_IPI,/*PesqPict("SC7","C7_IPI",16,mv_par12)*/PesqPict("SC7","C7_IPI")) ,oFont6c,100/*,,,2*/ ) 
oPrn:Say( li, CalcCol(2310)  , Transform(xMoeda(SC7->C7_TOTAL,SC7->C7_MOEDA,MV_PAR12,SC7->C7_DATPRF),PesqPict("SC7","C7_FRETE",14,mv_par12)) ,oFont6c,100/*,,,2*/ )
oPrn:Say( li, /*2433*/ CalcCol(2640) , DateToC(SC7->C7_DATPRF) ,oFont6c,100,,,2 )
oPrn:Say( li, /*2750*/ CalcCol(2920) , Transform( SC7->C7_CC   ,PesqPict("SC7","C7_CC",16,mv_par12)) ,oFont6c,100,,,2 ) 
//oPrn:Say( li, /*2750*/ CalcCol(2920) , Transform( MaFisRet(Val(SC7->C7_ITEM),"IT_VALSOL" )     ,PesqPict("SC7","C7_FRETE",16,mv_par12)) ,oFont6c,100,,,2 ) 
...
oPrn:Say( 1840, CalcCol(2050) , Transform(xMoeda(nTotIcmsSt,SC7->C7_MOEDA,MV_PAR12,SC7->C7_DATPRF),tm(nTotIcmsSt,14,MsDecimais(MV_PAR12))) ,oFont4c,100 )
...
CpyT2S( cAnexo, "\RELATO\" )

Sleep(5000)

cAnxSrv  := "\RELATO\" + AllTrim(cArq) + ".PDF"
...
oProcess:cTo 	  := "ljjbmg@gmail.com" //cMailFor//"teste@alpax.com.br"  
