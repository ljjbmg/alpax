...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
								...
								//VERIFICA SE A NOTA É DE ENTRADA OU SAÍDA e PEGA O E-MAIL E MONTA NOME DO ARQUIVO PARA ENVIO DA DANFE
									...
									oPrintPDF := Nil            
									lPreview := .T.
									oPrintPDF := FWMSPrinter():New(cNomArq+".PDF",6,.F.,,.T.,,,,,,,.F.)
...
For nZ := 1 To nLenDet
	...
	While !Empty(cAux) .Or. !Empty(cAuxItem) .Or. !Empty(aAux[1]) .Or. !Empty(aAux[2]) .Or. !Empty(aAux[3]) .Or. !Empty(aAux[4]) .Or. !Empty(aAux[5]) .Or. !Empty(aAux[6])
		...
		lPontilhado := .T.
	EndDo
	// incluido por Fagner / Biale 11/08/09
	ZZ := 1
	IF Type("oNf:_infnfe:_det:_Infadprod:TEXT")<>"U" .or. Type("oNf:_infnfe:_det[nPrivate]:_Infadprod:TEXT") <> "U"
	    
	    If substr(AllTrim(oDet[nX]:_Infadprod:TEXT),1,7) == 'Pedido:'//.AND. substr(AllTrim(oDet[nX]:_Infadprod:TEXT),1,8) <> 'Pedido:|'
	      	cAux2 := SEPARA(AllTrim(oDet[nX]:_Infadprod:TEXT)   ,"|"  )                                             
	   		aadd(aItens,{"",alltrim(cAux2[1]) +" " + alltrim(cAux2[2])+"-"+alltrim(cAux2[3]),"","","","","","","","","","","",""})
    	Else	
	    
	    	cAux2 := SEPARA(  AllTrim(oDet[nX]:_Infadprod:TEXT)   ,";"  )             
			_n:= 1
	
			For nCont := _n to len(cAux2) 
				If substr(alltrim(cAux2[nCont]),1,8) <> 'Pedido:|'  
					aadd(aItens,{"",Substr(alltrim(cAux2[nCont]),1,46),"","","","","","","","","","","",""}) 
					If Len(cAux2[nCont]) > 46
						aadd(aItens,{"",Substr(alltrim(cAux2[nCont]),47,94),"","","","","","","","","","","",""}) 	
					EndIf
					If Len(cAux2[nCont]) > 94
						aadd(aItens,{"",Substr(alltrim(cAux2[nCont]),95,140),"","","","","","","","","","","",""}) 	
					EndIf
...
For nX := 1 To nForTo
	oDanfe:Say(nLinCalc,098,SubStr(cStrAux,Iif(nX==1,1,((nX-1)*25)+1),25), oFont11N:oFont )
	nLinCalc+=10
Next nX
...
If Type("oEmitente:_EnderEmit:_xCpl") <> "U"
	...
	oDanfe:Say(nLinCalc,098, "Fone: "+IIf(Type("oEmitente:_EnderEmit:_Fone")=="U","",oEmitente:_EnderEmit:_Fone:Text),oFont08N:oFont)
	nLinCalc+=9
	oDanfe:Say(nLinCalc,098, "Site: www.alpax.com.br",oFont08N:oFont)
	nLinCalc+=9
